...
def templates():
    ...
    # Tab filtering: page | blog (removed 'all')
...
def export_templates():
    ...
    db = get_db()
    cursor = db.cursor()
    cursor.execute('SELECT id, title, slug, category, content, is_default, sort_order, created_at, updated_at FROM templates ORDER BY sort_order')
...
def export_selected_templates():
    ...
    cursor.execute(f'SELECT id, title, slug, category, content, is_default, sort_order, created_at, updated_at FROM templates WHERE id IN ({placeholders}) ORDER BY sort_order', ids)
    rows = cursor.fetchall()
...
def import_templates(import_data, overwrite_existing, cursor):
    ...
    for t in import_data:
        try:
            ...
            cursor.execute('SELECT id FROM templates WHERE slug = ?', (slug,))
            ...
            if sort_order is None:
                cursor.execute('SELECT COALESCE(MAX(sort_order), 0) FROM templates')
                sort_order = (cursor.fetchone()[0] or 0) + 1

            # Insert template
            cursor.execute('''
                INSERT INTO templates (title, slug, category, content, is_default, sort_order)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (
                t.get('title') or slug,
                slug,
            ...
            if t.get('is_default', 1):
                ...
                for p in pages:
                    # Put at end by current max sort order for the page
                    cursor.execute('SELECT COALESCE(MAX(sort_order), 0) FROM page_templates WHERE page_id = ?', (p['id'],))
                    max_order = cursor.fetchone()[0] or 0
                    cursor.execute('INSERT INTO page_templates (page_id, template_id, sort_order) VALUES (?, ?, ?)', (p['id'], new_template_id, max_order + 1))
...
def add_template():
    ...
    if request.method == 'POST':
        ...
        cursor.execute('SELECT id FROM templates WHERE slug = ?', (slug_input,))
        if cursor.fetchone():
            flash('Template with this slug already exists', 'error')
            return redirect(url_for('templates_.add_template'))

        # Get max sort order
        cursor.execute('SELECT MAX(sort_order) FROM templates')
        max_order = cursor.fetchone()[0] or 0
        ...
        if is_default:
            # Create page templates for all existing pages
...
def edit_template(template_id):
    ...
    if request.method == 'POST':
        ...
        cursor.execute('SELECT id FROM templates WHERE slug = ? AND id != ?', (slug_input or slugify(title), template_id))
        if cursor.fetchone():
            flash('Template with this slug already exists', 'error')
            return redirect(url_for('templates_.edit_template', template_id=template_id))
...
def delete_template(template_id):
    ...
    cursor.execute('SELECT title FROM templates WHERE id = ?', (template_id,))
    template = cursor.fetchone()
...
def move_template(template_id, direction):
    ...
    cursor.execute('SELECT sort_order, slug FROM templates WHERE id = ?', (template_id,))
    ...
    # Scope neighbor search to same group (page vs blog) to avoid jumping
    is_blog = current['slug'].startswith('blog_')
    scope_clause = "slug LIKE 'blog_%'" if is_blog else "slug NOT LIKE 'blog_%'"

    if direction == 'up':
        # Find the previous template
        cursor.execute(f'SELECT id, sort_order FROM templates WHERE {scope_clause} AND sort_order < ? ORDER BY sort_order DESC LIMIT 1',
                     (current['sort_order'],))
        prev_template = cursor.fetchone()

        if prev_template:
            # Swap sort orders
            cursor.execute('UPDATE templates SET sort_order = ? WHERE id = ?', (prev_template['sort_order'], template_id))
            cursor.execute('UPDATE templates SET sort_order = ? WHERE id = ?', (current['sort_order'], prev_template['id']))
        ...
        cursor.execute(f'SELECT id, sort_order FROM templates WHERE {scope_clause} AND sort_order > ? ORDER BY sort_order ASC LIMIT 1',
                     (current['sort_order'],))
        next_template = cursor.fetchone()

        if next_template:
            # Swap sort orders
            cursor.execute('UPDATE templates SET sort_order = ? WHERE id = ?', (next_template['sort_order'], template_id))
            cursor.execute('UPDATE templates SET sort_order = ? WHERE id = ?', (current['sort_order'], next_template['id']))
